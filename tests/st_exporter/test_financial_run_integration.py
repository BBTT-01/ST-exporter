...
from st_exporter.sales import ESTIMATE_COLUMNS
...
def test_writes_all_four_tabs_with_the_contract_headers(st_settings, exporter_settings) -> None:
    ...
    assert export_store.tabs[FINANCIAL_ESTIMATES_TAB][0] == list(ESTIMATE_COLUMNS)
    ...
    assert summary.financial_row_counts == {
        ...
        FINANCIAL_ESTIMATES_TAB: 2,  # one sold estimate's item + one unsold, itemless row
    }
...
def test_each_tab_gets_its_own_meta_row_with_the_contract_version(
    ...
    # `sales.estimates` runs on this feed's cadence but is a wholly new tab, so it
    # carries its own contract version (`sales.v1`), never `financial.v1` — see
    # `contracts.SALES`.
    expected_versions = {tab: "financial.v1" for tab in FINANCIAL_FEED_NAMES}
    expected_versions[FINANCIAL_ESTIMATES_TAB] = "sales.v1"
    for tab in FINANCIAL_FEED_NAMES:
        assert meta[tab].contract_version == expected_versions[tab]
...
def test_an_unmarked_namesake_report_is_refused_on_its_columns(
    ...
    # The other four tabs are untouched by one bad report.
    assert len(summary.financial_row_counts) == 4
